    ...
    for (size_t i = 0; i < max; ++i) {
        ...
        for (int t = 0; t < 2; ++t) {
            ...
            for (size_t n = 0; n < spks.size(); ++n) {
                ...
                BOOST_CHECK_EQUAL(IsSolvable(HodlCash(key_provider, script_provider), spks[n]), (flags & UNSOLVABLE) == 0);

                if (flags & SIGNABLE) {
                    CMutableTransaction spend;
                    spend.vin.resize(1);
                    spend.vout.resize(1);
                    BOOST_CHECK_MESSAGE(SignSignature(HodlCash(keys_priv, script_provider), spks[n], spend, 0, 1, SIGHASH_ALL), prv);
                }
