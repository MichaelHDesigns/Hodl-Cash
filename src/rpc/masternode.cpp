...
#include <node/context.h>   //! access to g_rpc_node
#include <rpc/blockchain.h> //! access to g_rpc_node
...
UniValue listmasternodes(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() > 1))
        throw std::runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode HODL address\n"
    ...
    for (const auto& s : vMasternodeRanks) {
        ...
        if (mn) {
            if (strFilter != "" && strTxHash.find(strFilter) == std::string::npos && mn->Status().find(strFilter) == std::string::npos && EncodeDestination(PKHash(mn->pubKeyCollateralAddress)).find(strFilter) == std::string::npos)
                continue;
...
UniValue getmasternodestatus(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() != 0))
        throw std::runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) HODL address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
UniValue getmasternodewinners(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) HODL MN Address\n"
...
UniValue decodemasternodebroadcast(const JSONRPCRequest& request)
{
    ...
    lastPingObj.pushKV("sigvalid", mnb.lastPing.VerifySignature(mnb.pubKeyMasternode, nDos) ? "true" : "false");
